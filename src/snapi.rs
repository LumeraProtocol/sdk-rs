...
async fn parse_json_or_sse(resp: reqwest::Response) -> Result<serde_json::Value, SdkError> {
    ...
    serde_json::from_str(&body).map_err(|e| SdkError::Serialization(format!("unsupported status body format: {e}; body={body}")))
}
...
impl SnApiClient {
    ...
    pub async fn start_cascade(&self, action_id: &str, signature: &str, file_path: &Path) -> Result<String, SdkError> {
    ...
    pub async fn request_download(&self, action_id: &str, signature: &str) -> Result<String, SdkError> {
        let resp = self.http
